...
function check_word() {
    let inputword = wordle.value.toLowerCase();

    wordlist.children[word_idx].textContent = inputword;

    let selected = wordlist.children[word_idx];
    let result = "";

    for (let i = 0; i < inputword.length; i++) {
        if (inputword[i] == randomWord[i]) {
            result += `<span class="highlight">${inputword[i]}</span>`;
        } else if (randomWord.includes(inputword[i])) {
            result += `<span class="yellow">${inputword[i]}</span>`;
        } else {
            result += inputword[i];
        }
    }
    
    selected.innerHTML = result;
    word_idx++;
    if (word_idx > 5) {window.alert(`the word was ${randomWord}!!`)}
